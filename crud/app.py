...
def index():
    ...
    try:
        cursor.execute("SELECT id, name, description FROM items ORDER BY id DESC")
...
def edit_item(item_id):
    ...
    try:
        ...
        item = cursor.fetchone() #In database programming, fetchone() is a method used to retrieve the next row (or record) from a result set. It returns a single row as a tuple, and if no more rows are available
    except Error as e:
